...
TARGET_DURATION = 2.5  # ~2.5 seconds of CPU burn
...
async def process_message(message: aio_pika.IncomingMessage, r: aioredis.Redis):
    ...
    async with message.process():
        try:
            ...
            logger.info(f"Worker C processing job {job_id} (simulation {simulation})")
            ...
            logger.info(f"Worker C completed job {job_id} in {elapsed:.2f}s")
...
async def main():
    logger.info(f"Worker C connecting to {BROKER_URL}")
    ...
    logger.info(f"Worker C listening on {QUEUE_NAME}")
    ...
    try:
        ...
        logger.error(f"Worker C error: {e}", exc_info=True)
    finally:
        await r.aclose()
