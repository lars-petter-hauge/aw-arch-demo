...
async def process_message(message: aio_pika.IncomingMessage, r: aioredis.Redis):
    ...
    async with message.process():
        try:
            ...
            logger.info(f"Worker A processing job {job_id} (iteration {iteration})")
            ...
            logger.info(f"Worker A completed job {job_id} in {elapsed:.2f}s")
...
async def main():
    logger.info(f"Worker A connecting to {BROKER_URL}")
    ...
    logger.info(f"Worker A listening on {QUEUE_NAME}")
    ...
    try:
        ...
        logger.error(f"Worker A error: {e}", exc_info=True)
    finally:
        await r.aclose()
